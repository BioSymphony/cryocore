#!/usr/bin/env python3
"""Generate the RunPod bridge manifest for the T2R14 open dossier demo."""
...
cat > /tmp/t2r14_open_dossier.py <<'PY'
{script_text}
PY
python3 /tmp/t2r14_open_dossier.py --out runpod-execution --json | tee runpod-execution/artifacts/demo-summary.json
"""
    return {
        "schema_version": 1,
        ...
        "workload": {
            "scale": "small",
            "description": "Public PDB/EMDB open-tool dossier for 2026 T2R14 cryo-EM structure",
            "shards": [{"shard_id": "single-dossier"}],
        ...
        "launch_authorization": {
            ...
            "scope": "Prep-only public data demo. Paid launch requires operator substitution and closeout.",
        },
        "budget": {"max_runtime_minutes": 45, "max_estimated_cost_usd": 1},
        "repo": {
            "source": "inline_commands",
            "url_or_path": "inline",
            "commit_or_snapshot": "inline:t2r14-open-dossier-v1",
        ...
        "runpod": {
            ...
            "templateId": "",
            "name": "cryocore-demo-t2r14-open-dossier",
            "gpuCount": 0,
            "gpuTypeIds": [],
            "dataCenterIds": [],
            "networkVolumeId": "",
        ...
        "worker_coordination": {
            ...
            "resource_name_prefix": "cryocore-demo-t2r14",
        },
        "closeout": {
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "runpod" / "bridge-manifests" / "t2r14-open-dossier.json")
    args = parser.parse_args()
    script = (ROOT / "scripts" / "cryocore" / "t2r14_open_dossier.py").read_text()
