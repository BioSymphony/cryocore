...
def check_stage_contract_report(
    root: Path,
    execution_mode: str,
    stage_progress_summary: dict[str, Any],
) -> tuple[dict[str, Any], list[str], list[str]]:
    path = root / "validation/stage-contract-check.json"
    errors: list[str] = []
    warnings: list[str] = []
    summary: dict[str, Any] = {
        "path": repo_relative(path),
    ...
    if not path.exists():
        if execution_mode == "real":
            errors.append("validation/stage-contract-check.json missing")
        return summary, errors, warnings
    try:
        report = load_json(path)
    except json.JSONDecodeError as exc:
        errors.append(f"stage contract report malformed: {exc.msg}")
        return summary, errors, warnings
    if not isinstance(report, dict):
        errors.append("stage contract report must be a JSON object")
        return summary, errors, warnings
    terminal_by_stage = report.get("terminal_by_stage", {})
    summary["ok"] = report.get("ok")
    summary["terminal_stage_count"] = len(terminal_by_stage) if isinstance(terminal_by_stage, dict) else 0
    if report.get("ok") is False:
        errors.append("stage contract report reports ok false")
    if execution_mode == "real" and not isinstance(terminal_by_stage, dict):
        errors.append("stage contract report terminal_by_stage must be an object")
    if execution_mode == "real" and isinstance(terminal_by_stage, dict) and not terminal_by_stage:
        errors.append("stage contract report has no terminal stage coverage")
    progress_terminal = stage_progress_summary.get("terminal_by_stage", {})
    if isinstance(progress_terminal, dict) and isinstance(terminal_by_stage, dict):
        missing = sorted(set(progress_terminal) - set(terminal_by_stage))
        if missing:
            warnings.append("stage contract report omits terminal progress stages: " + ", ".join(missing))
    return summary, errors, warnings
...
def check_cost_report(root: Path, paid_provider: bool, execution_mode: str) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    if paid_provider and execution_mode == "real" and total is None:
        errors.append("cost report does not include total_cost_usd")
    if paid_provider and execution_mode == "real" and max_authorized is None:
        errors.append("cost report does not include max_authorized_spend_usd")
    if paid_provider and execution_mode == "real" and not budget_status:
        errors.append("cost report does not include budget_status")
    return summary, errors, warnings
...
def normalize_hash_ledger(payload: Any) -> dict[str, str]:
    if isinstance(payload, dict) and isinstance(payload.get("sha256"), dict):
...
def check_hash_evidence(
    ...
    *,
    ...
    if existing is None:
        if require_ledger:
            errors.append("artifact hash ledger missing for real paid/provider closeout")
    else:
        try:
            ledger_hashes = normalize_hash_ledger(load_json(existing))
        except json.JSONDecodeError as exc:
            errors.append(f"artifact hash ledger malformed: {exc.msg}")
        if not ledger_hashes:
            errors.append("artifact hash ledger does not contain a sha256 mapping")
    missing_hashes: list[str] = []
    mismatched_hashes: list[str] = []
    if ledger_hashes:
        for rel, digest in computed_hashes.items():
            if rel not in ledger_hashes:
                missing_hashes.append(rel)
            elif ledger_hashes[rel] != digest:
                mismatched_hashes.append(rel)
        if require_ledger and missing_hashes:
            errors.append("artifact hash ledger missing required artifacts: " + ", ".join(sorted(missing_hashes)))
        if mismatched_hashes:
            errors.append("artifact hash ledger mismatches required artifacts: " + ", ".join(sorted(mismatched_hashes)))
    elif computed_hashes and not require_ledger:
        warnings.append("artifact hashes were computed but no hash ledger was present")
    return {
        ...
        "missing_from_ledger": sorted(missing_hashes),
        "mismatched_hashes": sorted(mismatched_hashes),
...
def load_shard_ledger(path: Path) -> dict[str, Any]:
    if path.suffix == ".jsonl":
        ...
        return {
            "schema_version": 1,
            "ledger_type": "cryocore_cloud_shard_ledger",
            "ledger_format": "jsonl",
            "shards": shards,
...
def validate_shard_ledger(path: Path, execution_mode: str) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    if ledger.get("ledger_type") not in {None, "cryocore_cloud_shard_ledger"}:
...
def evaluate_closeout(args: argparse.Namespace) -> dict[str, Any]:
    ...
    stage_contract_summary: dict[str, Any] = {}
    ...
    if args.artifact_root:
        ...
        if not artifact_root.exists():
            ...
            stage_contract_summary, stage_contract_errors, stage_contract_warnings = check_stage_contract_report(
                artifact_root,
                args.execution_mode,
                stage_summary,
            )
            errors.extend(stage_contract_errors)
            warnings.extend(stage_contract_warnings)
            ...
            require_hash_ledger = args.execution_mode == "real" and paid_provider
            hash_summary, hash_errors, hash_warnings = check_hash_evidence(
                artifact_root,
                artifact_checks,
                require_ledger=require_hash_ledger,
            )
    ...
    return {
        ...
        "stage_contract_report": stage_contract_summary,
