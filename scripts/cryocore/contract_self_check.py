#!/usr/bin/env python3
"""Final evidence join check for CryoCore runs.
...
def check_expected_artifacts(manifest: dict[str, Any], root: Path) -> list[str]:
    ...
    run_prefix = f"/workspace/cryocore/runs/{run_id}/"
...
def check_map_model_validation(root: Path, real_required: bool) -> list[str]:
    ...
    if real_required:
        ...
        if not isinstance(payload.get("source_hashes"), dict) or not payload.get("source_hashes"):
            errors.append("real map/model validation requires source_hashes")
        if not isinstance(payload.get("deposition_report_ids"), list) or not payload.get("deposition_report_ids"):
            errors.append("real map/model validation requires deposition_report_ids")
        if not isinstance(payload.get("caveats"), list) or not payload.get("caveats"):
            errors.append("real map/model validation requires caveats")
...
def check_workflow_provenance(manifest: dict[str, Any], root: Path, real_required: bool) -> list[str]:
    errors: list[str] = []
    provider = str(manifest.get("provider") or "").lower()
    if not real_required or provider in {"", "local"}:
        return errors
    workflow = maybe_load_json(root / "validation" / "workflow-run.json")
    container = maybe_load_json(root / "validation" / "container-provenance.json")
    if not workflow:
        errors.append("real remote closeout requires validation/workflow-run.json")
    if not container:
        errors.append("real remote closeout requires validation/container-provenance.json")
    if workflow:
        for key in ["schema_version", "run_id", "workflow_engine", "workflow_version", "workflow_ref", "profile", "executor", "trace_path", "output_root", "resume_mode", "exit_status"]:
            if is_missing_evidence(workflow.get(key)):
                errors.append(f"workflow-run.json missing {key}")
        if workflow.get("exit_status") not in {"completed", "partial", "blocked", "failed"}:
            errors.append(f"workflow-run.json has invalid exit_status: {workflow.get('exit_status')}")
    if container:
        for key in ["schema_version", "run_id", "container_runtime", "image", "image_digest", "sbom_path", "versions_path"]:
            if is_missing_evidence(container.get(key)):
                errors.append(f"container-provenance.json missing {key}")
        image_digest = str(container.get("image_digest") or "")
        sif_sha = str(container.get("sif_sha256") or "")
        if not image_digest.startswith("sha256:") and len(sif_sha) != 64:
            errors.append("container-provenance.json requires image_digest sha256:... or 64-character sif_sha256")
    return errors
...
def check_fanout_estimate(manifest: dict[str, Any], root: Path, real_required: bool) -> list[str]:
    errors: list[str] = []
    execution_profile = manifest.get("execution_profile") or manifest.get("environment", {}).get("CRYOCORE_EXECUTION_PROFILE")
...
def self_check(manifest_path: Path, artifact_root: Path, execution_mode: str) -> dict[str, Any]:
    ...
    errors.extend(check_workflow_provenance(manifest, artifact_root, real_required=execution_mode == "real"))
    ...
    return {
        "ok": not errors,
        "check_type": "cryocore_contract_self_check",
