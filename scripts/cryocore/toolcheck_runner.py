#!/usr/bin/env python3
"""Run no-download CryoCore smoke checks.
...
OPEN_TOOLS = {
    ...
    "cryodrgn": ["cryodrgn", "--help"],
    "recovar": ["recovar", "--help"],
...
def write_prep_stage_artifacts(manifest_path: Path, manifest: dict[str, Any], out: Path) -> None:
    ...
    if stage_contract is None:
        summary = {
            "ok": False,
            "check_type": "cryocore_stage_contract_check",
...
def storage_check(root: Path) -> dict[str, Any]:
    ...
    probe = root / ".cryocore_write_probe"
    payload = f"cryocore {datetime.now(timezone.utc).isoformat()}\n"
...
def main() -> int:
    ...
    volume_root = Path(
        os.environ.get("CRYOCORE_VOLUME_ROOT")
        or manifest.get("environment", {}).get("CRYOCORE_VOLUME_ROOT", "/workspace/cryocore")
    )
    if str(out).startswith(".runtime"):
        storage_root = out / "storage-probe"
    else:
        storage_root = volume_root / "runs" / manifest.get("run_id", "cryocore-smoke")
    ...
    provenance = f"""# CryoCore Smoke Provenance
