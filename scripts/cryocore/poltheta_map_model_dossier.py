#!/usr/bin/env python3
"""Build a real public map/model dossier for EMD-43816 / PDB 9ASJ.
...
PDB_ID = "9ASJ"
EMDB_ID = "EMD-43816"
RUN_ID = "cryocore-demo-poltheta-map-model-dossier"
...
ARCHIVE_ALLOWLIST = [
    ...
    "interchain-contact-matrix.json",
    "interchain-contact-examples.csv",
...
def fetch_binary(url: str, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    request = urllib.request.Request(url, headers={"User-Agent": "BioSymphony-CryoCore/0.1"})
...
def write_public_archive(out: Path) -> None:
...
def build_dossier(out: Path, skip_map_download: bool = False) -> dict[str, Any]:
    start = time.time()
    ...
    command_event(commands_path, "input_audit", "poltheta_map_model_dossier.py input_audit", ["validation/input-audit.json"], now())
    ...
    command_event(commands_path, "data_intake", "poltheta_map_model_dossier.py download_public_map_model_validation", ["data-intake-ledger.json", "data/"], now())
    ...
    command_event(commands_path, "model_analysis", "poltheta_map_model_dossier.py parse_mmcif_and_validation_xml", ["coordinate-summary.json", "ligand-neighborhoods.json"], now())

    stage_event(progress_path, "map_analysis", "started", "Parsing EMDB MRC header and density evidence")
    if not map_path.exists():
        raise RuntimeError("Map download was skipped; real map/model dossier cannot pass")
    ...
    command_event(commands_path, "map_analysis", "poltheta_map_model_dossier.py parse_mrc_map_and_sample_density", ["map-summary.json"], now())
    ...
    source_hashes = {record["path"]: record["sha256"] for record in materialized_files}
    map_model_fit = {
        "schema_version": 1,
        "ok": True,
        "evidence_level": "real_public_map_model_dossier",
        "pdb_id": PDB_ID,
        "emdb_id": EMDB_ID,
        ...
        "software_versions": {
            ...
            "parser": "BioSymphony CryoCore standard-library MRC/mmCIF/XML parser",
        },
        "source_hashes": source_hashes,
        "deposition_report_ids": [PDB_ID, EMDB_ID, "wwPDB validation 9ASJ"],
        "caveats": [
            "Density support is a fast deposited-map proxy, not full real-space refinement.",
            "Mechanism and ligand interpretation require expert review before publishable claims.",
        ],
    }
    ...
    command_event(commands_path, "validation_summary", "poltheta_map_model_dossier.py join_map_model_validation_evidence", ["validation/map_model_fit.json"], now())

    stage_event(progress_path, "figure_dossier", "started", "Writing reproducible SVG figure panels and reports")
    ...
    report_md = f"""# BioSymphony CryoCore Demo: Pol theta Map/Model Dossier
...
This dossier supports a real deposited-map/deposited-model public-data demo. It does not replace expert real-space refinement, local manual inspection, half-map reprocessing, or biological mechanism validation.
"""
    (out / "report.md").write_text(report_md)
    html_report = f"""<!doctype html>
<html><head><meta charset="utf-8"><title>BioSymphony Pol theta Map/Model Demo</title>
<style>body{{font-family:Arial,sans-serif;max-width:1040px;margin:32px auto;line-height:1.45}} img{{max-width:100%;border:1px solid #ddd;margin:8px 0 22px}} code{{background:#f4f4f4;padding:2px 4px}}</style></head>
<body>
<h1>BioSymphony CryoCore: Pol theta Map/Model Dossier</h1>
<p><strong>EMDB:</strong> {EMDB_ID} &nbsp; <strong>PDB:</strong> {PDB_ID} &nbsp; <strong>Reported resolution:</strong> {metadata['resolution_angstrom']} A</p>
<p>{html.escape(str(metadata['title']))}</p>
<h2>Figures</h2>
<h3>Deposited Map Orthoslice</h3><img src="figures/emd-43816-mid-slice.svg">
<h3>Model Inventory</h3><img src="figures/model-inventory.svg">
<h3>AMP-PNP Neighborhood</h3><img src="figures/ampnp-neighborhood.svg">
<h3>Density Support Checks</h3><img src="figures/density-support.svg">
<h2>Claim Boundary</h2>
<p>Real public map/model analysis with fast density-support checks. Mechanistic and publishability claims require expert review and stronger validation.</p>
</body></html>
"""
...
"""
    ...
    stage_event(progress_path, "figure_dossier", "completed", "Figure panels and report complete")
    command_event(commands_path, "figure_dossier", "poltheta_map_model_dossier.py render_svg_figures_and_report", ["figures/", "report.html", "report.md"], now())

    stage_event(progress_path, "claim_audit", "started", "Writing explicit claim ledger")
...
"""
    (out / "claim_ledger.md").write_text(claim_ledger)
    claim_ledger_json = {
        "claims": [
            ...
            {"claim": "Final mechanism is established", "claim_level": "insufficient_evidence", "evidence_artifact": "claim_ledger.md"},
        ]
    }
    write_json(out / "claim_ledger.json", claim_ledger_json)
    dossier_manifest = {
        "schema_version": 1,
        "run_id": RUN_ID,
    ...
    write_json(out / "dossier_manifest.json", dossier_manifest)
    contract_manifest = {
        "schema_version": 1,
        "run_id": RUN_ID,
        "execution_profile": "map-model-dossier",
    ...
    stage_event(progress_path, "claim_audit", "completed", "Claim ledger complete")
    ...
    command_event(
        commands_path,
        "claim_audit",
        "poltheta_map_model_dossier.py write_claim_ledger_and_contract_manifest",
        ["claim_ledger.md", "claim_ledger.json", "dossier_manifest.json", "contract-manifest.json", "validation/stage-contract-check.json"],
        now(),
    )
    ...
    write_public_archive(out)
    stage_event(progress_path, "archive", "completed", "Artifact archive ready")
    command_event(commands_path, "archive", "poltheta_map_model_dossier.py archive_artifacts", ["runpod-execution.tar.gz"], now())
...
def main() -> int:
    ...
    try:
        summary = build_dossier(args.out, skip_map_download=args.skip_map_download)
