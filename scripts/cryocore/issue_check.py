#!/usr/bin/env python3
"""Validate a CryoCore tracker issue draft."""
...
VALID_PROVIDERS = {"runpod", "local", "ssh-hpc", "generic-cloud", "neocloud", "aws-batch", "provider-neutral"}
...
VALID_ROUTING_LABELS = {"sym:cryocore"}
...
FORBIDDEN_WORKER_MUTATION_PATTERNS = (
    " --execute",
...
def normalize_reference(raw: str) -> str | None:
    ...
    if not ref or ref in {"n/a", "none"}:
        return None
    if " " in ref or ref.startswith(("http://", "https://", "s3://", "gs://", "/", "~")):
        return None
...
def validation_command_references(text: str) -> set[str]:
    refs = references_from_code_spans(text, VALIDATION_REFERENCE_PREFIXES)
    token_pattern = re.compile(r"(?<![A-Za-z0-9_./-])([A-Za-z0-9_./*-]+(?:/[A-Za-z0-9_./*-]+)+)")
    for line in text.splitlines():
        stripped = line.strip()
        if not stripped or stripped.startswith("#") or stripped.startswith(("test ", "[ ")):
            continue
...
def issue_file_references(text: str) -> set[str]:
    ...
    for heading in [
        ...
        refs.update(references_from_code_spans(sections.get(heading, ""), REPO_REFERENCE_PREFIXES))
...
def forbidden_validation_commands(text: str) -> list[str]:
    commands = markdown_sections(text).get("## Validation Commands", "")
    violations: list[str] = []
    for pattern in FORBIDDEN_WORKER_MUTATION_PATTERNS:
...
def check_issue(path: Path, repo_root: Path, check_file_references: bool) -> dict[str, object]:
    text = path.read_text()
    ...
    if "<!-- symphony:schema" not in text or "subgroup: cryocore" not in text:
        errors.append("missing cryocore symphony schema")
    if "```bash" not in text:
        errors.append("missing bash validation block")
    if "## Touched Areas" not in text or "- `" not in text:
    ...
    if path.name != "linear-issue.md":
        ...
        elif provider_match.group(1) not in VALID_PROVIDERS:
            errors.append(f"invalid provider: {provider_match.group(1)}")
        if execution_profile_match is None:
            errors.append("missing execution profile line")
        elif execution_profile_match.group(1) not in VALID_EXECUTION_PROFILES:
            errors.append(f"invalid execution profile: {execution_profile_match.group(1)}")
        if re.search(r"^- operator gate required:\s*`(yes|no)`\s*$", text, re.MULTILINE) is None:
            errors.append("missing operator gate required line")
        routing_label_match = re.search(r"^routing_label:\s*([\w:.-]+)\s*$", text, re.MULTILINE)
        declared_label = routing_label_match.group(1) if routing_label_match else "sym:cryocore"
        ...
        if "<exact command" in text or "<path>" in text or "<input id>" in text:
    ...
    for forbidden in forbidden_validation_commands(text):
        errors.append(f"validation commands contain provider-mutating worker command: {forbidden}")
    ...
    operator_gate_match = re.search(r"^- operator gate required:\s*`(yes|no)`\s*$", text, re.MULTILINE)
    gate_required = operator_gate_match and operator_gate_match.group(1) == "yes"
    if provider_value == "runpod" and gate_required and target_state_match and target_state_match.group(1) == "Todo":
        errors.append("operator-gated RunPod issues must not target Todo before the gate is approved")
    ...
    if check_file_references:
        ...
        missing_file_references = [ref for ref in checked_file_references if not reference_exists(repo_root, ref)]
    ...
    return {
        ...
        "has_cryocore_schema": "<!-- symphony:schema" in text and "subgroup: cryocore" in text,
        "has_validation_bash_block": "```bash" in text,
        "has_touched_area_entry": "## Touched Areas" in text and "- `" in text,
    }
...
def main() -> int:
    ...
    parser.add_argument("--check-file-references", action="store_true")
    ...
    if args.json:
        ...
        for item in summary["warnings"]:
            print(f"warning: {item['issue']}: {item['warning']}")
