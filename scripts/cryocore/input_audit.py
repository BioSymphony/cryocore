#!/usr/bin/env python3
"""Audit declared CryoCore inputs before asking for operator input.
...
def audit_launch_manifest(path: Path) -> dict[str, Any]:
    ...
    if isinstance(download_plan, dict) and download_plan:
        ...
        if not truthy_env("CRYOCORE_OPERATOR_AUTHORIZED"):
            missing_operator_items.append(
                {
                    ...
                    "env_or_artifact": "CRYOCORE_OPERATOR_AUTHORIZED=1",
                }
            )
    ...
    return {
        "ok": not errors,
        "audit_type": "cryocore_input_audit",
