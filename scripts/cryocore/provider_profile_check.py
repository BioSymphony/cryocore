#!/usr/bin/env python3
"""Validate CryoCore provider profiles."""
...
def validate_profile(data: dict[str, Any], path: Path) -> dict[str, Any]:
    ...
    if data.get("provider") == "runpod":
        ...
        if data.get("workspace_root") != "/workspace/cryocore":
            errors.append("RunPod workspace_root must be /workspace/cryocore")
        if not str(data.get("artifact_root", "")).startswith("/workspace/cryocore/runs/"):
            errors.append("RunPod artifact_root must be under /workspace/cryocore/runs/")
    else:
