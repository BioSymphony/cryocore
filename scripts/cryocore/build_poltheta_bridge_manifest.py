#!/usr/bin/env python3
"""Generate the RunPod bridge manifest for the Pol theta map/model demo."""
...
def build_manifest(runner_text: str, self_check_text: str, artifact_contract_text: str) -> dict:
    bundle_b64 = encoded_source_bundle(
        {
            "scripts/cryocore/poltheta_map_model_dossier.py": runner_text,
            "scripts/cryocore/contract_self_check.py": self_check_text,
            "modules/artifact-contracts/structure-dossier.v1.json": artifact_contract_text,
        }
    )
    workload_command = f"""mkdir -p scripts/cryocore modules/artifact-contracts runpod-execution/artifacts
python3 - <<'PY'
import base64
import io
import tarfile
...
PY
python3 scripts/cryocore/poltheta_map_model_dossier.py --out runpod-execution/artifacts --json | tee runpod-execution/artifacts/demo-summary.json
python3 scripts/cryocore/contract_self_check.py --manifest runpod-execution/artifacts/contract-manifest.json --artifact-root runpod-execution/artifacts --execution-mode real --json | tee runpod-execution/artifacts/validation/contract-self-check.stdout.json
python3 - <<'PY'
import tarfile
from pathlib import Path
...
PY
if [ -s runpod-execution/live-http.pid ]; then
  kill "$(cat runpod-execution/live-http.pid)" >/dev/null 2>&1 || true
  sleep 1
fi
"""
    return {
        ...
        "workload": {
            "scale": "small",
            "description": "Public EMDB/PDB map-model dossier for Pol theta helicase domain EMD-43816 / PDB 9ASJ",
            "shards": [{"shard_id": "single-map-model-dossier"}],
            ...
            "stage_contract": {
                ...
                "exact_commands": [
                    "python3 scripts/cryocore/poltheta_map_model_dossier.py --out runpod-execution/artifacts --json",
                    "python3 scripts/cryocore/contract_self_check.py --manifest runpod-execution/artifacts/contract-manifest.json --artifact-root runpod-execution/artifacts --execution-mode real --json",
        ...
        "launch_authorization": {
            ...
            "scope": "Prep-only public EMDB/PDB map-model demo. Paid launch requires operator substitution and closeout.",
        },
        "budget": {"max_runtime_minutes": 120, "max_estimated_cost_usd": 1},
        "repo": {
            "source": "inline_commands",
            "url_or_path": "inline",
            "commit_or_snapshot": "inline:poltheta-map-model-dossier-v1",
        ...
        "runpod": {
            ...
            "templateId": "",
            "name": "cryocore-demo-poltheta-map-model",
            "gpuCount": 0,
            "gpuTypeIds": [],
            "dataCenterIds": [],
            "networkVolumeId": "",
        ...
        "worker_coordination": {
            ...
            "resource_name_prefix": "cryocore-demo-poltheta",
        },
        "closeout": {
...
def main() -> int:
    ...
    parser.add_argument("--tracker-issue-url", default=TRACKER_ISSUE_URL)
    parser.add_argument("--out", type=Path, default=ROOT / "runpod" / "bridge-manifests" / "poltheta-map-model-dossier.json")
    args = parser.parse_args()
    runner = (ROOT / "scripts" / "cryocore" / "poltheta_map_model_dossier.py").read_text()
    self_check = (ROOT / "scripts" / "cryocore" / "contract_self_check.py").read_text()
    artifact_contract = (ROOT / "modules" / "artifact-contracts" / "structure-dossier.v1.json").read_text()
    manifest = build_manifest(runner, self_check, artifact_contract)
    manifest["launch_authorization"]["tracker_issue_url"] = args.tracker_issue_url
