#!/usr/bin/env python3
"""Build a small public cryo-EM map/model dossier for PDB 9W0Q.
...
def build_dossier(out: Path) -> dict[str, Any]:
    ...
    stage_event(stage_progress_path, "claim_audit", "started", "Writing validation and claim ledger")
    map_model_fit = {
        "schema_version": 1,
        "ok": True,
        ...
        "source_hashes": {f"{PDB_ID}.cif": sha256(data_dir / f"{PDB_ID}.cif")},
        "deposition_report_ids": [PDB_ID, str(metadata["emdb_id"])],
        "caveats": [
            "Map was not downloaded for this small public-coordinate demo.",
            "Geometry summaries are candidate observations, not mechanism proof.",
        ],
    }
    ...
    write_json(
        validation / "stage-contract-check.json",
        {
            ...
            "errors": [],
            "warnings": ["Small demo does not download the EM map; map-model validation claims are downgraded."],
        },
    )
...
| The dossier computed real chain inventories and inter-chain contact summaries from downloaded coordinates. | processed | `interchain-contact-matrix.json`, `figures/*.svg` | Contact thresholds are geometric heuristics, not mechanistic proof. |
...
"""
    (artifacts / "claim_ledger.md").write_text(claim_ledger)
    methods = f"""# Methods
...
"""
    ...
    report = f"""# BioSymphony CryoCore Demo: T2R14 Open Dossier
...
This is a real public-coordinate dossier, not a full map validation run. It supports artifact execution, coordinate-derived summaries, and candidate interface/ligand-neighborhood observations. It does not establish final mechanism or publishable density interpretation.
"""
    (artifacts / "report.md").write_text(report)
    html_report = f"""<!doctype html>
<html><head><meta charset="utf-8"><title>BioSymphony T2R14 Demo</title>
<style>body{{font-family:Arial,sans-serif;max-width:980px;margin:32px auto;line-height:1.45}} img{{max-width:100%;border:1px solid #ddd}} code{{background:#f4f4f4;padding:2px 4px}}</style></head>
<body>
<h1>BioSymphony CryoCore Demo: T2R14 Open Dossier</h1>
<p><strong>PDB:</strong> {PDB_ID} &nbsp; <strong>EMDB:</strong> {html.escape(str(metadata['emdb_id']))} &nbsp; <strong>Reported resolution:</strong> {metadata['resolution_angstrom']} A</p>
<p>{html.escape(str(metadata['title']))}</p>
<h2>Figures</h2>
<h3>Inter-chain Contacts</h3><img src="figures/interchain-contact-heatmap.svg">
<h3>Chain Inventory</h3><img src="figures/chain-residue-inventory.svg">
<h3>Ligand Neighborhoods</h3><img src="figures/ligand-neighborhoods.svg">
<h2>Claim Boundary</h2>
<p>Real public-coordinate analysis. No raw movies, no map download, no licensed tools. Mechanism claims are intentionally downgraded.</p>
</body></html>
"""
    (artifacts / "report.html").write_text(html_report)
    dossier_manifest = {
        "schema_version": 1,
        "run_id": RUN_ID,
        "pdb_id": PDB_ID,
    ...
    write_json(artifacts / "dossier_manifest.json", dossier_manifest)
    write_json(
        out / "status.json",
        {
            "ok": True,
    ...
    stage_event(stage_progress_path, "claim_audit", "completed", "Dossier and claim ledger complete")
