#!/usr/bin/env python3
"""Estimate CryoCore lane fanout before expensive execution.
...
def estimate_raw_subset(manifest: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    return {
        "ok": not blockers,
        "check_type": "cryocore_fanout_estimate",
        ...
        "shard_ledger_policy": {
            ...
            "expected_runtime_path": f".runtime/{manifest.get('run_id', 'cryocore-raw-subset')}/validation/cloud-shard-ledger.json",
            ...
            "validator": "scripts/cryocore/provider_closeout_check.py --shard-ledger <ledger> --execution-mode prep --json",
        },
        "blockers": blockers,
        "warnings": warnings,
    }
...
def estimate(manifest_path: Path) -> dict[str, Any]:
    ...
    if execution_profile not in {"raw-subset-open", "raw-subset-gated"}:
        return {
            ...
            "shard_ledger_policy": {
                "ledger_type": "cryocore_cloud_shard_ledger",
                "expected_runtime_path": f".runtime/{manifest.get('run_id', 'cryocore-run')}/validation/cloud-shard-ledger.json",
                "canary_required_before_paid_fanout": True,
                "provider_success_is_not_scientific_success": True,
                "per_shard_closeout_requires": RAW_SHARD_CLOSEOUT_ARTIFACTS,
                "validator": "scripts/cryocore/provider_closeout_check.py --shard-ledger <ledger> --execution-mode prep --json",
