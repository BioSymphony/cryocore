#!/usr/bin/env python3
"""Generate the RunPod bridge manifest for the two-target structure jury demo."""
...
try:
    from source_bundle import encoded_source_bundle
except ModuleNotFoundError:
    from scripts.cryocore.source_bundle import encoded_source_bundle
...
def build_manifest(files: dict[str, str]) -> dict:
    ...
    workload_command = f"""mkdir -p scripts/cryocore runpod-execution/artifacts
python3 - <<'PY'
import base64
import io
import tarfile
...
with tarfile.open(fileobj=io.BytesIO(base64.b64decode(payload)), mode="r:gz") as archive:
    for member in archive.getmembers():
        ...
        if source is None:
            raise RuntimeError(f"Unreadable bundle member: {{member.name}}")
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(source.read())
PY
python3 scripts/cryocore/structure_jury_dossier.py --out runpod-execution --json | tee runpod-execution/artifacts/campaign-summary.stdout.json
"""
    return {
        "schema_version": 1,
        ...
        "run_id": RUN_ID,
        "compute_profile": "cpu-public-structure-jury-under-2h",
        "workload": {
            "scale": "small",
            "description": "Two-target public deposited-structure jury for T2R14 and pol theta",
            "shards": [{"shard_id": "dual-public-dossier"}],
            ...
            "stage_contract": {
                ...
                "exact_commands": [
                    "python3 scripts/cryocore/structure_jury_dossier.py --out runpod-execution --json",
                ],
                "expected_outputs": [
                    "runpod-execution/status.json",
                    "runpod-execution/artifacts/campaign-summary.json",
                    "runpod-execution/artifacts/report.html",
                    "runpod-execution/artifacts/dossier_manifest.json",
                    ...
                    "runpod-execution/artifacts/targets/t2r14/artifacts/report.html",
                    "runpod-execution/artifacts/targets/t2r14/artifacts/figures/ligand-neighborhoods.svg",
                    "runpod-execution/artifacts/targets/poltheta/artifacts/report.html",
                    "runpod-execution/artifacts/targets/poltheta/artifacts/figures/density-support.svg",
                    "runpod-execution/artifacts/claim_ledger.md",
                ...
                "route_proof": {
                    "input_materialization": [
                        ...
                        "runpod-execution/artifacts/validation/input-audit.json records no missing operator items before target dossier execution",
                    ],
                    "tool_invocation": [
                        "python3 scripts/cryocore/structure_jury_dossier.py --out runpod-execution --json",
                        "runpod-execution/artifacts/executed-commands.jsonl records campaign_input_audit, t2r14_dossier, poltheta_dossier, campaign_join, claim_audit, and archive stages",
                    ],
                    "artifact_validation": [
                        "validation command reads runpod-execution/status.json, runpod-execution/artifacts/campaign-summary.json, runpod-execution/artifacts/validation/contract-self-check.json, runpod-execution/artifacts/dossier_manifest.json, and runpod-execution/artifacts/data-intake-ledger.json",
                        "expected artifacts include runpod-execution/artifacts/report.html, runpod-execution/artifacts/figures/evidence-matrix.svg, runpod-execution/artifacts/targets/poltheta/artifacts/validation/map_model_fit.json, and runpod-execution/artifacts/runpod-execution.tar.gz",
                    ],
                    "claim_boundaries": [
                        "runpod-execution/artifacts/claim_ledger.md must include insufficient_evidence boundaries for final mechanism and publication-ready validation",
                        "campaign claim level remains candidate for deposited public-data dossier evidence; raw reconstruction and licensed-tool refinement are out of scope",
                    ],
                },
            },
        },
        ...
        "launch_authorization": {
            ...
            "approved_by": "operator-required",
            "approved_at": "",
            "tracker_issue_url": TRACKER_ISSUE_URL,
            "scope": "Prep-only dual public PDB/EMDB structure jury. Paid launch requires operator substitution and closeout.",
        },
        "budget": {"max_runtime_minutes": 120, "max_estimated_cost_usd": 2},
        "repo": {
            "source": "inline_commands",
            "url_or_path": "inline",
            "commit_or_snapshot": "inline:structure-jury-dual-dossier-v1",
        ...
        "runpod": {
            ...
            "templateId": "",
            "name": "cryocore-demo-structure-jury",
            "gpuCount": 0,
            "gpuTypeIds": [],
            "dataCenterIds": [],
            "networkVolumeId": "",
            ...
            "env": {"CRYOCORE_RUN_ID": RUN_ID},
        },
        "access": {
            "ssh_required": False,
            "full_ssh_scp_required": False,
            "http_proxy_required": True,
        ...
        "startup": {
            ...
            "inspection": {"hold_after_success_seconds": 600, "http_artifact_server_port": 8000},
            "commands": [
                "set -euo pipefail",
                "cd /workspace/repo",
                "mkdir -p runpod-execution/logs runpod-execution/artifacts && { nohup python3 -m http.server \"${RUNPOD_HTTP_ARTIFACT_SERVER_PORT:-8000}\" --bind 0.0.0.0 --directory runpod-execution/artifacts >> runpod-execution/logs/live-http.log 2>&1 </dev/null & echo $! > runpod-execution/live-http.pid; disown \"$(cat runpod-execution/live-http.pid)\" 2>/dev/null || true; }",
                workload_command,
            ],
        },
        "monitoring": {
        ...
        "expected_artifacts": [
            ...
            {"artifact_id": "report_html", "path": "runpod-execution/artifacts/report.html", "required": True, "sha256_required": True},
            {"artifact_id": "report_md", "path": "runpod-execution/artifacts/report.md", "required": True, "sha256_required": True},
            {"artifact_id": "dossier_manifest", "path": "runpod-execution/artifacts/dossier_manifest.json", "required": True, "sha256_required": True},
            ...
            {"artifact_id": "t2r14_report", "path": "runpod-execution/artifacts/targets/t2r14/artifacts/report.html", "required": True, "sha256_required": True},
            ...
            {"artifact_id": "poltheta_report", "path": "runpod-execution/artifacts/targets/poltheta/artifacts/report.html", "required": True, "sha256_required": True},
            ...
            {"artifact_id": "claim_ledger", "path": "runpod-execution/artifacts/claim_ledger.md", "required": True, "sha256_required": True},
        ...
        "worker_coordination": {
            ...
            "resource_name_prefix": "cryocore-demo-structure-jury",
        },
        "closeout": {
        ...
        "safety": {
            ...
            "license_policy": "open tooling only; no CryoSPARC/Phenix/ChimeraX or other gated tools",
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "runpod" / "bridge-manifests" / "structure-jury-dual-dossier.json")
    args = parser.parse_args()
    files = {
        "scripts/cryocore/structure_jury_dossier.py": (ROOT / "scripts" / "cryocore" / "structure_jury_dossier.py").read_text(),
        "scripts/cryocore/t2r14_open_dossier.py": (ROOT / "scripts" / "cryocore" / "t2r14_open_dossier.py").read_text(),
        "scripts/cryocore/poltheta_map_model_dossier.py": (ROOT / "scripts" / "cryocore" / "poltheta_map_model_dossier.py").read_text(),
    }
