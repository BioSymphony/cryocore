#!/usr/bin/env python3
"""Build a two-target public structure jury dossier.
...
emits a campaign-level claim audit. It still avoids raw movies, private data,
...
from scripts.cryocore import poltheta_map_model_dossier as poltheta  # noqa: E402
from scripts.cryocore import t2r14_open_dossier as t2r14  # noqa: E402


RUN_ID = "cryocore-demo-structure-jury-dual-dossier"
TOP_LEVEL_ARCHIVE_ALLOWLIST = [
...
TARGET_ARCHIVE_ALLOWLIST = [
    "status.json",
    "artifact_hashes.json",
    "artifacts/coordinate-summary.json",
    "artifacts/interchain-contact-matrix.json",
    "artifacts/interchain-contact-examples.csv",
    "artifacts/ligand-neighborhoods.json",
    "artifacts/map-summary.json",
    "artifacts/dossier_manifest.json",
    "artifacts/data-intake-ledger.json",
    "artifacts/executed-commands.jsonl",
    "artifacts/stage-progress.jsonl",
    "artifacts/contract-manifest.json",
    "artifacts/claim_ledger.md",
    "artifacts/claim_ledger.json",
    "artifacts/methods.md",
    "artifacts/provenance.md",
    "artifacts/report.md",
    "artifacts/report.html",
    "artifacts/validation/input-audit.json",
    "artifacts/validation/map_model_fit.json",
    "artifacts/validation/stage-contract-check.json",
    "artifacts/figures/interchain-contact-heatmap.svg",
    "artifacts/figures/chain-residue-inventory.svg",
    "artifacts/figures/ligand-neighborhoods.svg",
    "artifacts/figures/emd-43816-mid-slice.svg",
    "artifacts/figures/model-inventory.svg",
    "artifacts/figures/ampnp-neighborhood.svg",
    "artifacts/figures/density-support.svg",
]
...
def write_public_archive(artifacts: Path) -> None:
    include = list(TOP_LEVEL_ARCHIVE_ALLOWLIST)
    for target_id in ["t2r14", "poltheta"]:
        include.extend(f"targets/{target_id}/{rel}" for rel in TARGET_ARCHIVE_ALLOWLIST)
...
def write_evidence_matrix_svg(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    parts = [
        ...
        '<text x="24" y="34" font-family="Arial" font-size="22" font-weight="700">Structure Jury Evidence Matrix</text>',
...
def write_maturity_svg(path: Path, levels: list[dict[str, str]]) -> None:
    ...
    parts = [
        ...
        '<text x="28" y="60" font-family="Arial" font-size="13" fill="#444">The campaign reaches L5 for deposited public-data dossiers; raw reconstruction remains a separate future lane.</text>',
    ]
...
def target_summary(label: str, root: Path, artifacts: Path) -> dict[str, Any]:
    ...
    manifest = read_json(artifacts / "dossier_manifest.json")
    claim_text = (artifacts / "claim_ledger.md").read_text()
    ...
    return {
        ...
        "figures": figures,
        "claim_ledger_has_insufficient_evidence": "insufficient_evidence" in claim_text,
...
def build_self_check(artifact_root: Path, manifest: dict[str, Any], targets: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for target in targets:
        ...
        if not target["claim_ledger_has_insufficient_evidence"]:
            errors.append(f"target claim ledger lacks insufficient_evidence boundary: {target['label']}")
        if target["figure_count"] <= 0:
            errors.append(f"target has no figures: {target['label']}")
    if manifest.get("execution_profile") != "map-model-dossier":
        errors.append("campaign manifest must use execution_profile map-model-dossier")
    ...
    return {
        "ok": not errors,
        "check_type": "structure_jury_contract_self_check",
        "execution_mode": "real",
        "run_id": RUN_ID,
        ...
        "errors": errors,
        "warnings": [
            "This is a public deposited-data structure jury, not raw cryo-EM reconstruction.",
            "T2R14 is coordinate/metadata-only in this campaign; pol theta includes deposited map sampling.",
        ],
    }


def build_dossier(out: Path) -> dict[str, Any]:
    start = time.time()
    ...
    stage_event(progress_path, "campaign_input_audit", "started", "Declaring public structure jury inputs")
    input_audit = {
        "ok": True,
        "known_inputs": [
        ...
        "execution_scope": "public deposited PDB/EMDB dossiers only; no raw movies; no license-gated tools",
    }
    write_json(validation / "input-audit.json", input_audit)
    command_event(commands_path, "campaign_input_audit", "structure_jury_dossier.py declare_public_inputs", ["validation/input-audit.json"], now())
    stage_event(progress_path, "campaign_input_audit", "completed", "Input audit complete")

    stage_event(progress_path, "t2r14_dossier", "started", "Running T2R14 public-coordinate dossier")
    ts = now()
    t2r14_root = targets_root / "t2r14"
    t2r14_result = t2r14.build_dossier(t2r14_root)
    command_event(commands_path, "t2r14_dossier", "t2r14_open_dossier.py --out artifacts/targets/t2r14 --json", ["targets/t2r14/status.json", "targets/t2r14/artifacts/"], ts)
    stage_event(progress_path, "t2r14_dossier", "completed", "T2R14 dossier complete")

    stage_event(progress_path, "poltheta_dossier", "started", "Running pol theta public map/model dossier")
    ts = now()
    poltheta_root = targets_root / "poltheta"
    poltheta_artifacts = poltheta_root / "artifacts"
    poltheta_result = poltheta.build_dossier(poltheta_artifacts)
    command_event(commands_path, "poltheta_dossier", "poltheta_map_model_dossier.py --out artifacts/targets/poltheta/artifacts --json", ["targets/poltheta/status.json", "targets/poltheta/artifacts/"], ts)
    stage_event(progress_path, "poltheta_dossier", "completed", "Pol theta dossier complete")
    ...
    map_model_fit = {
        "schema_version": 1,
        "ok": True,
        "evidence_level": "joined_public_deposited_structure_jury",
        ...
        "source_hashes": {item["path"]: item["sha256"] for item in data_files if item.get("sha256")},
        "deposition_report_ids": ["9W0Q", "EMD-65512", "9ASJ", "EMD-43816", "wwPDB validation 9ASJ"],
        "caveats": [
            "Joined jury includes one map-sampled target and one coordinate/metadata target.",
            "Campaign-level claims remain processed/candidate until expert density review.",
        ],
    }
    ...
    campaign_summary = {
        "ok": True,
        "run_id": RUN_ID,
        "execution_profile": "map-model-dossier",
        "target_count": len(targets),
        "targets": targets,
    ...
    command_event(commands_path, "campaign_join", "structure_jury_dossier.py join_target_evidence", ["campaign-summary.json", "data-intake-ledger.json", "validation/map_model_fit.json"], now())

    stage_event(progress_path, "claim_audit", "started", "Writing campaign-level claim audit")
    manifest = {
        "schema_version": 1,
        "run_id": RUN_ID,
        "execution_profile": "map-model-dossier",
    ...
    write_json(artifacts / "dossier_manifest.json", manifest)
    claim_json = {
        "claims": [
            {
                "claim": "Two independent public structure lanes were executed and joined into one campaign dossier.",
            ...
            },
            {
                ...
                "evidence_artifact": "claim_ledger.md",
            },
        ]
    }
    write_json(artifacts / "claim_ledger.json", claim_json)
    claim_md = """# Structure Jury Claim Ledger

| Claim | Level | Evidence | Caveat |
| --- | --- | --- | --- |
| Two independent public structure lanes ran and were joined into one dossier. | processed | `campaign-summary.json`, `executed-commands.jsonl` | This proves orchestration and artifact joining, not raw reconstruction. |
...
"""
    ...
    report_md = """# BioSymphony CryoCore: Two-Target Structure Jury

This campaign runs two real public structure lanes and joins the evidence into one claim-audited dossier.
...
This is a deposited public-data structure jury. It proves input materialization, exact-command execution, artifact joining, and claim auditing across two lanes. It does not claim raw-data reconstruction or final biological mechanism.
"""
    (artifacts / "report.md").write_text(report_md)
    report_html = """<!doctype html>
<html><head><meta charset="utf-8"><title>BioSymphony Structure Jury</title>
<style>body{font-family:Arial,sans-serif;max-width:1080px;margin:32px auto;line-height:1.45} img{max-width:100%;border:1px solid #ddd} code{background:#f4f4f4;padding:2px 4px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}</style></head>
<body>
<h1>BioSymphony CryoCore: Two-Target Structure Jury</h1>
<p><strong>Targets:</strong> PDB 9W0Q / EMD-65512 and PDB 9ASJ / EMD-43816.</p>
<p>Real public deposited-data inputs, exact command ledgers, joined artifacts, and explicit claim boundaries. No raw movies, private data, or license-gated tools.</p>
<h2>Campaign Evidence</h2>
<img src="figures/evidence-matrix.svg">
<img src="figures/maturity-ladder.svg">
<h2>Target Reports</h2>
<div class="grid">
<div><h3>T2R14</h3><p><a href="targets/t2r14/artifacts/report.html">Open target dossier</a></p><img src="targets/t2r14/artifacts/figures/ligand-neighborhoods.svg"></div>
<div><h3>Pol theta</h3><p><a href="targets/poltheta/artifacts/report.html">Open target dossier</a></p><img src="targets/poltheta/artifacts/figures/density-support.svg"></div>
</div>
<h2>Claim Boundary</h2>
<p>This campaign reaches a claim-audited deposited-data dossier, not raw reconstruction or final mechanism validation.</p>
</body></html>
"""
    (artifacts / "report.html").write_text(report_html)
    ...
    stage_contract_check = {
        "ok": self_check["ok"],
        "check_type": "structure_jury_stage_contract_check",
        "require_terminal": True,
        "terminal_by_stage": {
            "campaign_input_audit": "completed",
            "t2r14_dossier": "completed",
            "poltheta_dossier": "completed",
    ...
    stage_event(progress_path, "claim_audit", "completed", "Campaign claim audit complete")
    command_event(commands_path, "claim_audit", "structure_jury_dossier.py write_campaign_claim_audit", ["dossier_manifest.json", "claim_ledger.md", "validation/contract-self-check.json"], now())
    ...
    write_public_archive(artifacts)
    stage_event(progress_path, "archive", "completed", "Artifact archive ready")
    command_event(commands_path, "archive", "structure_jury_dossier.py archive_artifacts", ["runpod-execution.tar.gz"], now())
    status = {
        "ok": self_check["ok"],
        "run_id": RUN_ID,
...
def main() -> int:
    ...
    try:
        summary = build_dossier(args.out)
