#!/usr/bin/env python3
"""Validate a CryoCore RunPod launch manifest."""
...
def validate(manifest: dict[str, Any], *, manifest_path: Path | None = None, execution_ready: bool = False) -> dict[str, Any]:
    ...
    execution_profile = manifest.get("execution_profile") or env.get("CRYOCORE_EXECUTION_PROFILE")
    is_no_download = execution_profile == "no-download-smoke" or env.get("CRYOCORE_NO_DOWNLOAD") in {"1", "true", "yes"}
    ...
    if is_no_download:
        ...
        if env.get("CRYOCORE_NO_DOWNLOAD") not in {"1", "true", "yes"}:
            errors.append("CRYOCORE_NO_DOWNLOAD must be set to a truthy value for no-download profile")
    else:
        if env.get("CRYOCORE_NO_DOWNLOAD") in {"1", "true", "yes"}:
            errors.append("CRYOCORE_NO_DOWNLOAD must not be truthy outside no-download profile")
