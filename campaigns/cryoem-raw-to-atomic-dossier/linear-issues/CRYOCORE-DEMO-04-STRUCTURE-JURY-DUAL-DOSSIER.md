...
may generate `.runtime/*launch-request.json` in prep mode; the trusted host-side
`after_run` hook performs provider mutation, artifact fetch/hash, and cleanup.
...
- `scripts/cryocore/structure_jury_dossier.py` - public two-target campaign runner.
- `scripts/cryocore/build_structure_jury_bridge_manifest.py` - RunPod bridge manifest builder.
...
touched_areas:
  ...
  - scripts/cryocore/structure_jury_dossier.py
  - scripts/cryocore/build_structure_jury_bridge_manifest.py
