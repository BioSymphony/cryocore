...
Harden CryoCore provider adapter contracts so RunPod remains the reference remote path while local, SSH/HPC, generic cloud VM, and neocloud profiles can share the same BioSymphony evidence contract.
...
- `scripts/cryocore/provider_profile_check.py` - provider profile validator.
- `scripts/cryocore/runpod_scope_check.py` - CryoCore RunPod resource boundary validator.
...
- writable volume/env: `provider-specific; RunPod uses CRYOCORE_RUNPOD_NETWORK_VOLUME_ID`
...
- [ ] RunPod profiles are marked as the reference remote path.
...
- [ ] RunPod bridge manifests fail validation if they reference sibling campaign pods, volumes, templates, or generic writable volume variables.
...
- `scripts/cryocore/` - provider profile validator.
- `tests/` - provider validation coverage.

## Dependencies

Blocked by: CRYOCORE-W12
...
- Keep provider-specific secrets out of git and tracker comments.
