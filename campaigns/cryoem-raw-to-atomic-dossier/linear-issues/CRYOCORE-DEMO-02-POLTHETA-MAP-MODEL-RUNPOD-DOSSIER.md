...
- `scripts/cryocore/poltheta_map_model_dossier.py` - public map/model dossier runner.
- `scripts/cryocore/build_poltheta_bridge_manifest.py` - RunPod bridge manifest builder.
- `runpod/bridge-manifests/poltheta-map-model-dossier.json` - audited launch manifest.
- `scripts/cryocore/contract_self_check.py` - real-route self-check hardening.
...
Blocked by: operator approval for this public map/model RunPod execution and operator-owned RunPod API access.

## Risk Notes

- Do not store RunPod credentials in git, tracker comments, artifacts, or reports.
- Do not use CryoSPARC, Phenix, ChimeraX, or other license-gated tools.
...
touched_areas:
  ...
  - scripts/cryocore/poltheta_map_model_dossier.py
  - scripts/cryocore/build_poltheta_bridge_manifest.py
