...
sf_progress_path() {
  local run_id="${CRYOCORE_RUN_ID:-cryocore-run}"
  local volume_root="${CRYOCORE_VOLUME_ROOT:-/workspace/cryocore}"
...
sf_stage_event() {
  ...
  local status="$2"
  local message="${3:-}"
  local progress_path="${CRYOCORE_STAGE_PROGRESS:-$(sf_progress_path)}"
