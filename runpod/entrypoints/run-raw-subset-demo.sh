...
RUN_ID="${CRYOCORE_RUN_ID:-cryocore-raw-subset}"
...
if [[ "${MANIFEST}" == *raw-subset-gated* ]]; then
  STAGE_CONTRACT="${CRYOCORE_STAGE_CONTRACT:-${REPO_ROOT}/runpod/stage-contracts/raw-subset-gated.stage-contract.json}"
fi
EXECUTION_MODE="${CRYOCORE_EXECUTION_MODE:-prep}"
...
if [[ ! -f "${REPO_ROOT}/scripts/cryocore/toolcheck_runner.py" && -n "${CRYOCORE_REPO_URL:-}" ]]; then
  bash "${ENTRYPOINT_DIR}/bootstrap-repo.sh"
fi

if [[ ! "${CRYOCORE_ALLOW_RAW_DOWNLOADS:-0}" =~ ^(1|true|yes)$ ]]; then
  echo "raw subset execution blocked: CRYOCORE_ALLOW_RAW_DOWNLOADS must be truthy" >&2
  sf_stage_event "operator_authorization" "failed" "CRYOCORE_ALLOW_RAW_DOWNLOADS not truthy"
  sf_partial_summary "operator_authorization" "blocked" "set CRYOCORE_ALLOW_RAW_DOWNLOADS after explicit operator approval and rerun" "none"
  exit 2
fi

if [[ ! "${CRYOCORE_OPERATOR_AUTHORIZED:-0}" =~ ^(1|true|yes)$ ]]; then
  echo "raw subset execution blocked: CRYOCORE_OPERATOR_AUTHORIZED must be truthy" >&2
  sf_stage_event "operator_authorization" "failed" "CRYOCORE_OPERATOR_AUTHORIZED not truthy"
  sf_partial_summary "operator_authorization" "blocked" "set CRYOCORE_OPERATOR_AUTHORIZED after explicit operator approval and rerun" "none"
  exit 2
fi

CURRENT_STAGE="manifest_preflight"
sf_stage_start "${CURRENT_STAGE}" "validating launch manifest"
python3 "${REPO_ROOT}/scripts/cryocore/runpod_manifest_check.py" "${MANIFEST}" --json \
...
if [[ "${MANIFEST}" == *raw-subset-gated* ]]; then
  ...
  python3 "${REPO_ROOT}/scripts/cryocore/license_gate_check.py" \
    --manifest "${MANIFEST}" \
...
python3 "${REPO_ROOT}/scripts/cryocore/input_audit.py" \
  --manifest "${MANIFEST}" \
...
python3 "${REPO_ROOT}/scripts/cryocore/fanout_estimator.py" \
  --manifest "${MANIFEST}" \
...
python3 "${REPO_ROOT}/scripts/cryocore/toolcheck_runner.py" \
  --manifest "${MANIFEST}" \
  --out "${RUN_ROOT}" \
  ${CRYOCORE_MOCK_GPU:+--mock-gpu}
...
cat > "${RUN_ROOT}/data-intake-ledger.json" <<JSON
{
  ...
  "subset_profile": "${CRYOCORE_SUBSET_PROFILE:-raw_movies_100}",
...
python3 "${REPO_ROOT}/scripts/cryocore/contract_self_check.py" \
  --manifest "${MANIFEST}" \
...
python3 "${REPO_ROOT}/scripts/cryocore/stage_contract_check.py" \
